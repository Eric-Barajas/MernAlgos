...
function groupObjects(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        const lowerCaseCat = items[i].category.toLowerCase();
        if (results[lowerCaseCat]) {
            // if(items[i].category in results){ //check if category exists in results array
            results[lowerCaseCat].push(items[i])
        } else { //add category into results, add item into that category
            results[lowerCaseCat] = [items[i]]
            // let obj ={
            //   [items[i].category] : items[i]
            // }
            // results.push(obj);
            // results.push([items[i].category] : items[i])
